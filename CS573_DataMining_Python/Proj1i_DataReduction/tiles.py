...
def tiles(table, numdim, outfile):
    ntable = projections.projections(table, numdim)
    tile = Tile()
...
def tiles4(x0, x2, y0, y2, ntable, tile, centable, c1, pre, outfile):
    x = x0 + int((x2 - x0)/2)
    y = y0 + int((y2 - y0)/2)
    c1 = tile1(x0, x, y0, y, ntable, tile, centable, c1, pre, outfile)
    c1 = tile1(x0, x, y+1, y2, ntable, tile, centable, c1, pre, outfile)
    c1 = tile1(x+1, x2, y0, y, ntable, tile, centable, c1, pre, outfile)
    c1 = tile1(x+1, x2, y+1, y2, ntable, tile, centable, c1, pre, outfile)
    return c1

def tile1(x0, x2, y0, y2, table, tile, centable, c1, pre, outfile):
    has = []
    for x in range(x0, x2+1):
        for y in range(y0, y2+1):
            if tile.xs[x-1]['d'] == tile.ys[y-1]['d']:
                has += [tile.xs[x-1]['d']]
    # debug info 
    if tile.watch: outfile.write('%3s:  '%c1+ ' '+str(pre)+' '+ str(x0)+' '+ str(x2)+' '+ str(y0)+' '+ str(y2)+ ' #'+ str(len(has))+'\n')
    # recurse: when there is enough data
    if len(has) >= tile.big:
        pre = pre + '|..'
        return tiles4(x0, x2, y0, y2, table, tile, centable,c1, pre, outfile)
